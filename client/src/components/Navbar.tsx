...
const technologiesMenu = tecnologias.map((t) => ({ href: `/tecnologias#${t.id}`, label: t.label, resumo: t.resumo }));
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            <div className="relative group">
              <Link
                href="/tecnologias"
                className={`nav-link text-sm font-['Inter'] font-semibold transition-colors duration-200 flex items-center gap-1 ${
                  location === "/tecnologias" ? "text-white" : "text-white/70 hover:text-white"
                }`}
              >
                Tecnologias
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="transition-transform group-hover:rotate-180">
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 z-50 opacity-0 invisible translate-y-1 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-200">
                <div className="bg-[#0a0a0f]/95 backdrop-blur-2xl rounded-2xl border border-[rgba(127,49,184,0.3)] p-3 flex flex-col gap-1 w-[340px] shadow-2xl shadow-black/50">
                  {technologiesMenu.map((item) => (
                    <a
                      key={item.href}
                      href={item.href}
                      onClick={() => track("technology_click", { placement: "navbar_menu", tecnologia: item.href })}
                      className="px-3 py-2 rounded-lg hover:bg-[rgba(127,49,184,0.15)] transition-colors"
                    >
                      <span className="block text-sm font-['Inter'] font-semibold text-white/90">{item.label}</span>
                      <span className="block text-xs font-['Inter'] text-white/50 leading-snug mt-0.5">{item.resumo}</span>
                    </a>
                  ))}
                  <a
                    href="/tecnologias"
                    onClick={() => track("technology_click", { placement: "navbar_menu_ver_todas" })}
                    className="px-3 py-2 rounded-lg text-sm font-['Inter'] font-semibold text-[#F45504] hover:bg-[rgba(244,85,4,0.1)] transition-colors mt-1 border-t border-[rgba(255,255,255,0.08)] pt-3"
                  >
        ...
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute top-0 right-0 w-[280px] h-full bg-black border-l border-[rgba(127,49,184,0.3)] p-8 pt-24 flex flex-col gap-6 overflow-y-auto animate-in slide-in-from-right duration-300">
            ...
            <div>
              <Link
                href="/tecnologias"
                className={`text-lg font-['Inter'] font-bold ${location === "/tecnologias" ? "text-white" : "text-white/70"}`}
              >
                Tecnologias
              </Link>
              <div className="mt-3 pl-3 border-l border-[rgba(127,49,184,0.3)] flex flex-col gap-2">
